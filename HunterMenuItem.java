...
public class HunterMenuItem extends Effect { 
	public HunterMenuItem(GameState game, Point position)
	{
		super(game,position); //calling the tower superclass constructor
		
		image = ResourceLoader.getLoader().getImage("hunter.png"); //giving it an image
	}
	@Override
	public void update() {
		...
		if(game.getButtonActionPending() && game.getMousePos().distance(position)<50&&game.getCredits()>=50 && game.getTowerMoving()) //if the mouse is clicked and is near the object image on the menu
		{
			...
			game.addPending(new HunterMoving(game,position)); //send in a new tower at the same position
